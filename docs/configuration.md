...
| `TRUSS_TRUSTED_PROXIES` | Comma-separated IPs or CIDR ranges whose `X-Forwarded-For` / `X-Real-IP` headers are trusted, used to recover the real client IP for rate limiting (default: empty, so the TCP peer address is used). Required for per-client rate limiting behind a reverse proxy or CDN; see [Rate Limiting](deployment.md#rate-limiting) |
...
The server echoes the request ID back in the `X-Request-Id` response header, making it easy to correlate client-side logs with server-side entries. To propagate your own trace context, send an `X-Request-Id` header with your request and the server will reuse it. An incoming value is reused only when it is printable ASCII (0x20-0x7E) and at most 128 characters, which covers UUIDs and W3C `traceparent`; anything else is replaced by a generated UUID, because the value is reflected into a response header, where anything outside that range is something a proxy in front of truss is entitled to reject under RFC 9110.
