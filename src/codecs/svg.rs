...
use crate::core::{
    Artifact, ArtifactMetadata, MediaType, TransformError, TransformRequest, TransformResult,
    MAX_OUTPUT_PIXELS,
};
...
/// (JPEG, PNG, WebP, AVIF), the SVG is rasterized using `resvg` and encoded into the
/// target format.
///
/// # Errors
///
...
pub fn transform_svg(request: TransformRequest) -> Result<TransformResult, TransformError> {
    ...
    let (width, height) = determine_render_size(
        &tree,
    ...
    let bytes = encode_raster_output(&rgba_image, normalized.options.format, normalized.options.quality)?;
        ...
        match reader.read_event() {
            ...
            Ok(Event::Text(ref e)) => {
                ...
                if in_style {
                    ...
                    let text_event =
                        quick_xml::events::BytesText::new(&sanitized_css);
                    writer
...
fn is_event_handler(attr_name: &str) -> bool {
    ...
    lower.starts_with("on")
        && lower.len() > 2
        && lower.as_bytes()[2].is_ascii_alphabetic()
}
...
fn sanitize_css_urls(css: &str) -> String {
    ...
    while let Some(start) = remaining.to_ascii_lowercase().find("url(") {
        ...
        let trimmed = url_value.trim().trim_matches(|c| c == '\'' || c == '"').trim();
...
mod tests {
    use super::*;
    use crate::core::{sniff_artifact, RawArtifact, TransformOptions};
    ...
    fn sanitize_removes_script_element() {
        ...
        assert!(!result.contains("<script"), "script element should be removed");
    ...
    fn sanitize_removes_event_handlers() {
        ...
        assert!(result.contains("<rect"), "rect element should be preserved");
        assert!(result.contains("width"), "width attribute should be preserved");
    }
    ...
    fn sanitize_preserves_valid_svg() {
        ...
        assert!(result.contains("<rect"), "rect element should be preserved");
        assert!(result.contains("fill=\"blue\""), "fill attribute should be preserved");
    }
    ...
    fn sanitize_removes_external_css_url() {
        ...
        assert!(result.contains("url()"), "dangerous url() should be emptied");
    }
