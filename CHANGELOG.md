...
- An output that is not a regular file is written through rather than replaced ([#440](https://github.com/nao1215/truss/issues/440)). v0.20.0 made a write atomic by renaming a temporary file over the destination, which is right for an image being overwritten and wrong for everything else a path can name, because a rename unlinks whatever was there. `truss convert src.png -o pipe.png` on a named pipe turned the pipe into a 92 byte regular file, exited 0, and left the process reading the pipe blocked with nothing delivered; `truss convert src.png -o latest.png` on a symbolic link unlinked the link and left the file it pointed at untouched, where every other tool follows the link and updates the target. A character device survived only because `/dev` is not writable and the existing fallback caught it, which is the shape of the omission: the fallback asked whether a temporary file could be created beside the destination, never what the destination was. The replacement now runs for a path that names nothing yet or names a regular file, decided with `symlink_metadata` so a link is seen as a link, and every other destination gets the direct write it did before v0.20.0. Those destinations give up the partial-write protection, which they could never have had.

- A read-only output file is refused rather than overwritten ([#441](https://github.com/nao1215/truss/issues/441)). A rename asks the directory for permission and never asks the file, so once the write became atomic in v0.20.0 a mode of `0400` on the destination stopped nothing: `truss convert src.png -o readonly.png` exited 0, the contents changed, and the old mode was copied onto the replacement so the file still read `0400` afterwards. Making a file read-only is how a caller marks it as not to be touched, and a batch converting a directory of images is where that mark is meant to hold; before v0.20.0 the same command failed with `Permission denied` and named the file. The destination is now opened for writing before the replacement is attempted, which asks the kernel the question the kernel used to answer, so a file the caller may not write is reported with exit 2 and left alone. Nothing changes for a writable destination, and a file in a directory the caller cannot write to is still replaced, which is what the file's own mode decides on Unix.

- A number outside the range an option documents is refused with that range, whatever the number's width ([#437](https://github.com/nao1215/truss/issues/437)). The option's storage type was doing the validating before the documented rule ran, so a caller learned a different limit depending on which side of an implementation detail their typo landed on: `--quality 255` was answered `quality must be between 1 and 100` and `--quality 256` was answered `256 is not in 0..=255`, the span of a `u8`; the server answered the same pair `quality must be between 1 and 100` and `invalid value: integer 256, expected u8`, and the public query route answered `query parameter \`quality\` must be an integer`, which 256 is. That is the rule v0.15.0 settled when it stopped printing `without_enlargement` and `preserve_exif` at callers, one level further down: `u8`, `u32`, `i32`, and `0..=255` are none of the four adapters' vocabulary and none of them are the numbers `docs/openapi.yaml` and `truss help convert` publish. Every entry point now parses wide and asks one predicate in the core, so the sentence is the same at 101, at 255, and at 256, and `--watermark-opacity` gets the same treatment. A value that the option can hold is still judged where it was judged before, so no failure class or exit code changes; only a value too large to be held, which used to produce the integer's own message, produces the documented one instead.

- `--rotate` takes any whole number of degrees, including one that does not fit an `i32` ([#437](https://github.com/nao1215/truss/issues/437)). The flag documents that an angle past a full turn wraps, and `truss convert --rotate 9999999999` was refused with `unsupported rotation \`9999999999\`: expected a whole number of degrees`, which is what it is; the real limit was the parse type, and nothing said so. The server refused the same value with `expected i32`, so the two adapters disagreed about a value the option's own documentation accepts. The angle is now reduced to a single turn before it is narrowed, on both, so 9999999999 degrees is the 279 degrees it wraps to and a value that genuinely is not a whole number is still refused with the message that says so. `docs/openapi.yaml` already published `rotate` as an integer with no bound, so the implementation is what moved.

- A body that is valid JSON is not described as one that is not ([#438](https://github.com/nao1215/truss/issues/438)). Every `serde_json` failure was prefixed `request body must be valid JSON`, so `{"options":{"width":"wide"}}`, which parses by any measure, sent the caller looking for a syntax error that was not there while the actual problem was in a value. `serde_json` already separates the two through `classify`, and the two places that parse a body, the JSON route and the `options` part of a multipart upload, now read one helper that picks the sentence: a malformed document still gets `must be valid JSON`, and a document that parses and does not match gets `does not match the expected shape`. The detail after the colon is unchanged, so nothing that was diagnosable stops being diagnosable.

- The `--` section of `truss convert --help` prints command lines that run ([#434](https://github.com/nao1215/truss/issues/434)). It is the only place truss explains how to name a file that starts with a dash, and both examples it gave were malformed: `truss convert -- -input.png -o out.jpg` fails with `unexpected argument '-o' found`, because everything after `--` is a positional and the output flag is stranded among them, and `truss convert input.png -o -- -output.jpg` fails with `the argument '--output <OUTPUT>' cannot be used multiple times`, because `-o` takes the `--` as its value and `-output.jpg` then parses as the short cluster `-o utput.jpg`. A caller who reads the help because their shell has already been confused by the filename is told twice that their command is malformed. The section now prints `truss convert -o out.jpg -- -input.png` and `truss convert input.png --output=-output.jpg`, which work and which match what the README recommends for the same problem, and it says why an output cannot be escaped with `--` at all. `e2e/atago/help.atago.yaml` had twelve scenarios asserting the help contains particular strings and none that ran anything it prints, which is how two wrong command lines shipped; the examples are executed now, with the malformed form pinned as the usage error it is.

- The warning about a missing public base URL is asked of the configuration the server runs with ([#435](https://github.com/nao1215/truss/issues/435)). It was emitted inside `ServerConfig::from_env`, before `resolve_server_config` overlays the `serve` flags, so it read values the server does not use and came out inverted with respect to those flags: `truss serve --public-base-url https://cdn.example.com` with keys in the environment printed the warning although the base URL was set and in effect, and `truss serve --signed-url-key-id mykey --signed-url-secret s3cret`, which is the example printed by `truss serve --help`, printed nothing although signing was on with no authority configured. The second is the case the warning exists for: behind a proxy the Host header is not the authority the URL was signed for, so every signed URL is answered 401 with nothing saying why. The check now runs at the end of `resolve_server_config`, beside the storage check that already ran there, and at the same point in `truss validate`, whose whole job is to report a configuration fault before a port is bound and which takes no flags to overlay. It names both ways to set the value rather than only the environment variable. Nothing changes for a server configured entirely through the environment, which is the only case that behaved correctly before.

- The CLI checks every hop of a `--url` redirect, so a remote server cannot send it to a cloud metadata endpoint ([#430](https://github.com/nao1215/truss/issues/430)). `truss convert --url` and `truss inspect --url` validated the URL the caller typed and then let the HTTP agent follow the chain wherever it led, so a fixture answering `302 Location: http://169.254.169.254/latest/meta-data/` had truss connect to the link-local address and wait out its ten second connect timeout; the HTTP server, given the same redirect, refuses the target with `403 remote URL points to a cloud metadata service`, because it validates each hop and the metadata check is the one rule `TRUSS_ALLOW_INSECURE_URL_SOURCES` does not relax. The shape that matters is a build step running `truss convert --url "$IMAGE_URL"` on a URL from a CMS or a webhook: whoever controls that URL controls where truss connects, one redirect later. The CLI now follows redirects itself rather than inside the agent, checking each target and stopping after five, the same limit the server allows. The address rules the two adapters share moved into the core so there is one copy of them; the deny-list for private and loopback ranges stays the server's, because a command line fetching `http://localhost:3000/preview.png` during development is a thing people do, and no workflow fetches an image from a metadata endpoint. A host name that resolves to a metadata address without spelling it is refused too, since the addresses it resolves to are checked alongside the name; a host whose answer changes between that lookup and the agent's own is not covered, which the server closes with DNS pinning and can only do because it refuses every private address.

- An input truss cannot identify is reported by its length rather than by its bytes ([#431](https://github.com/nao1215/truss/issues/431)). The message carried the first sixteen bytes of the input in hexadecimal, which is a useful thing to say about a file the operator named and a disclosure about a URL fetched on somebody else's behalf: `truss convert --url` printed `unknown file signature (74 bytes, header: [41 57 53 5f 53 45 43 52 45 54 5f 41 43 43 45 53])`, which reads `AWS_SECRET_ACCES`, and the HTTP server returned the same string to the caller in the `detail` of its RFC 9457 body. On a default server the deny-list bounds what a caller can point truss at, with the exception that matters, a host on a public address that admits the server and not the caller; with `TRUSS_ALLOW_INSECURE_URL_SOURCES` set, which `docs/configuration.md` tells operators to set for MinIO, fake-gcs-server, or Azurite, there is no bound at all. What it yielded was existence, exact length, and sixteen leading bytes, which is reconnaissance rather than a credential dump and is still more than a caller of an image endpoint is owed. `sniff_artifact` builds the message and cannot tell where the bytes came from, so it now says neither: the message is `unknown file signature (74 bytes)` on every adapter. The length stays because it separates an empty response from a wrong one, and `xxd -l 16` is what a caller diagnosing a local file wants anyway.

- The worker pool is the transform limit plus headroom rather than the larger of the two, so a saturated server still answers a health probe ([#426](https://github.com/nao1215/truss/issues/426)). A worker is dedicated to a connection from accept to close, and the pool was `max(max_concurrent_transforms, 8)`, which is the transform limit itself for any limit of eight or more; since v0.20.0 the limit defaults to the machine's core count, so every host with eight cores or more had no worker left over. Measured on a 32-core machine with the limit at eight and twenty-four concurrent transforms, `GET /health/live` answered 200 after 10.97 seconds and `/health/ready` and `/metrics` took the same, while the same burst against a limit of seven, one below the constant, answered all three in about two milliseconds. `docs/deployment.md` names `/health/live` as the liveness probe, and a probe answered eleven seconds late is a probe that has already failed: Kubernetes restarts the container after its failure threshold, so the load the concurrency limit exists to survive was the load that killed the pod. The pool is now the limit plus eight, which costs eight threads waiting on a channel and leaves eight workers for every request that is not a transform, including the ones a cache hit answers. `docs/deployment.md` also described `/health/ready` as returning 503 only when draining, disk full, or over the memory limit, and omitted the `transformCapacity` check that reports fail while every slot is taken, which is the condition an operator meets first.

- A transform beyond the concurrency limit is refused rather than made to wait ([#427](https://github.com/nao1215/truss/issues/427)). v0.20.0 turned the excess away at admission with a 503 that says retry, and the guard sat behind the same pool: `TransformSlot::try_acquire` runs inside a worker, and with as many workers as slots a request could not reach it until a slot was already free, so the refusal never fired. Twenty-four concurrent transforms against eight slots produced twenty-four 200 responses, the excess after up to 12.4 seconds, where seven slots produced seventeen 503 responses in about five milliseconds each; what limited concurrency was an unbounded channel of accepted connections, which is queueing rather than admission control, and it told the caller nothing while it held a file descriptor per waiting client. With the headroom above, an excess request reaches the guard and is answered `too many concurrent transforms; retry later` before any work is done. Nothing changes below eight, where the behaviour was already this.

- A request that waited for a worker is logged and measured with the wait ([#428](https://github.com/nao1215/truss/issues/428)). `latency_ms` in the JSON access log and `truss_http_request_duration_seconds` share one start, and it was taken after the request's headers had been parsed, which is after the connection left the accept queue. The three probes above, which a client measured at 10.97 seconds, were logged as `"latency_ms":0` and recorded in the histogram as zero, so the backlog a saturated server actually has was invisible in both of the places an operator looks; `docs/prometheus.md` calls the histogram end-to-end HTTP request duration. The instant of accept now travels with the connection and starts the clock for the first request on it, which covers the wait for a worker and the header read the deadline already bounds. Later requests on a keep-alive connection waited for no worker, and the idle time before them belongs to no request, so they are measured from where they were.
...
- `cover` with `--without-enlargement` returns the requested box intersected with the source, and the documentation now says so ([#432](https://github.com/nao1215/truss/issues/432)). The fit table in `docs/pipeline.md` gives `cover` as "exactly the box, cropped at `position`", and the paragraph below it names the two modes the flag changes, `fill` and `contain`, without naming this one. It changes this one: a 100x50 source asked for 200x200 comes back 100x50, and asked for 200x40 it comes back 100x40, a ratio the source never had, because a crop cannot return more canvas than there is content. Thirteen of twenty combinations across a grid of four sources and five boxes come out smaller than the box, so a grid of thumbnails built with `--fit cover --width 200 --height 200 --without-enlargement` is ragged rather than square. The behaviour is unchanged and is what the implementation has always done deliberately; `docs/pipeline.md`, `docs/openapi.yaml`, and `truss help convert` now describe it, and a unit test pins the one-axis case the existing coverage did not reach.
