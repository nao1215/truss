...
BASE_URL="http://${AZURE_HOST}:${AZURE_PORT}/devstoreaccount1"
...
req = urllib.request.Request('${BASE_URL}/${CONTAINER}?restype=container', data=b'', method='PUT')
try:
...
upload() {
  local key="$1"
  local file="$2"
  local url="${BASE_URL}/${CONTAINER}/${key}"
  local status
  status=$(python3 -c "
import urllib.request, sys
